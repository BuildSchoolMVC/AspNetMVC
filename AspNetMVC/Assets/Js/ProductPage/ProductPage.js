...
function createCard() {
    ...
    if (serviceitemorginal.length==0)
    {
        toastr.warning("請勾選服務項目")
        return
    }
    else {
...
function fliterCardByRoomType() {
    ...
    let roomtypevalue = document.querySelector('input[name="roomtype"]:checked')
    if (roomtypevalue == null) {
        toastr.warning("請選擇空間類型")
    }
    else {
        cardarray.forEach(x => {
...
function fliterCardBySquareFeet() {
    showAllCard()
    let cardarray = document.getElementsByName("card");
    let squarefeetvalue = document.querySelector('input[name="squarefeet"]:checked')
    if (squarefeetvalue == null) {
        toastr.warning("請選擇空間大小")
    }
    cardarray.forEach(x => {
    ...
    allcartbtn.forEach(x => x.onclick = function (evt) {
        ...
        if (!document.cookie.includes("user")) {
            ...
            setTimeout(() => {
                ...
                $ball.style.top = temppageY - 5 + "px";
                $ball.style.left = temppageX - 10 + "px";
