  ...
  public class AccountService
  {
    ...
    public OperationResult EmailActivation(Guid? id)
    {
      ...
      try
      {
        ...
        if (user != null)
        {
          if (!JsonConvert.DeserializeObject<EmailStatus>(user.EmailStatus).EmailVerification)
          {
    ...
    public async Task<OperationResult> GetFacebookInfo(string code)
    {
      using (HttpClient client = new HttpClient())
      {
        ...
        try
        {
          var getAccessTokenUrl = $"https://graph.facebook.com/v9.0/oauth/access_token?client_id={WebConfigurationManager.AppSettings["Facebook_id"]}";
          ...
          var getUserInfoUrl = $"https://graph.facebook.com/me?access_token={result.Access_token}";
    ...
    public OperationResult GetMicrosoftInfo(string code)
    {
      ...
      try
      {
        string client_id = WebConfigurationManager.AppSettings["Microsoft_client_id"];
        string client_secret = WebConfigurationManager.AppSettings["Microsoft_client_secret"];
        string redirecr_url = $"{WebConfigurationManager.AppSettings["WebsiteUrl"]}Account/MicrosoftLogin";



        var token = isRock.Toolbox.MicrosoftSSO.Helper.GetTokenFromCode(code, client_id, client_secret, redirecr_url);
        var userinfo = isRock.Toolbox.MicrosoftSSO.Helper.GetUserInfo(token.access_token);
        
        or.IsSuccessful = true;
        or.MessageInfo = userinfo.userPrincipalName;
      }
      catch (Exception ex)
      {
        or.IsSuccessful = false;
    ...
    public Account GetUser(string email, string socialPlatform,bool isThirdParty)
    {
      return _repository.GetAll<Account>().FirstOrDefault(x => x.Email == email && x.SocialPlatform == socialPlatform && x.IsThirdParty == isThirdParty);
    }
  }
