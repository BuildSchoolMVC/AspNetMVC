...
using AspNetMVC.Repository.FakeData;
...
namespace AspNetMVC.Controllers
{
  public class AccountController : Controller
  {
    ...
    public ActionResult Login()
    {
      ...
      {
        ViewBag.ButtonList = new SocialButtonRepository().CreateButtonList(1);
        return View();
      }
    }
    ...
    public ActionResult Register()
    {
      ...
      else
      {
        ...
        ViewBag.ButtonList = new SocialButtonRepository().CreateButtonList(0);
        return View();
      }
    }
    ...
    public async Task<ActionResult> GoogleLogin(string token, int type)
    {
      ...
      if (result.IsSuccessful)
      {
        ...
        {
          var cookie = Helpers.SetCookie(_accountService.GetUser(googleTokenInfo.Email,"Google",true).AccountName, false);
          Response.Cookies.Add(cookie);
    ...
    public ActionResult FacebookLogin(string email, string socialPlatform, string imgUrl, int type)
    {
      ...
      {
        var cookie = Helpers.SetCookie(_accountService.GetUser(email,"Facebook",true).AccountName, false);
        Response.Cookies.Add(cookie);
    ...
    public ActionResult LineLogin(string code, int state)
    {
      ...
      if (result.IsSuccessful)
      {
        ...
        {
          var cookie = Helpers.SetCookie(_accountService.GetUser(user.Email,"Line",true).AccountName, false);
          Response.Cookies.Add(cookie);
    ...
    public ActionResult MicrosoftLogin(string code, string state)
    {
      var result = _accountService.GetMicrosoftInfo(code);

      if (result.IsSuccessful)
      {
        string email = result.MessageInfo;

        if (_accountService.IsSocialAccountRegister(email, "Microsoft"))
        {
          var cookie = Helpers.SetCookie(_accountService.GetUser(email, "Microsoft", true).AccountName, false);
          Response.Cookies.Add(cookie);
        ...
        {
          return RedirectToAction("SocialRegister", "Account", new { email = email, photo = "", social = "Microsoft" });
        }
      }
      else
      {
    ...
    public ActionResult SocialRegister(SocialRegisterViewModel model)
    {
      ...
      if (result.IsSuccessful)
      {
        _checkoutService.CreateCouponDetail(model.AccountName, CouponNum.新朋友享好禮);
